...
### 単一リポジトリの結論は一般化しない(★ v1.82.0、実測で 3 件覆った)
- 3 リポジトリ(groupcache 11年 / gorilla-mux 11.7年 / logrus 12.5年)で再計測した結果、
  **v0.119-0.128 の結論のうち 3 つが覆った**。詳細 `docs/MULTIREPO_FINDINGS.md`。
- **自リポジトリの lift 6.27-7.34× は外れ値**(実プロジェクトは 1.30-2.84×)。
  リリース儀式が「最大 × 最頻変更」のファイルを作るため、どの順位付けも当たる。
  **CHANGELOG の lift 値は yagura 固有の値として読むこと。**
- **precision@K は小規模リポジトリで飽和する**(新規に判明した限界)。ファイル 17-53 個・
  baseline 0.66-0.79 では全 scorer が precision 1.000 で同点になる。
  **lift が 1.0 付近なのを「差が無い」と読んではいけない**——正しくは
  「この規模ではこの指標が差を検出できない」。
- **v0.128.0 の coupling 結論はリポジトリ固有だった**: logrus では baseline に勝つ
  (1.32×)。機序(リリース儀式が頻度 baseline を強くする)は v0.128.0 が既に
  指摘していたが、反実仮想を示せていなかった。
- sliding window は 3 リポジトリで一度も勝たず、**expanding が単調に最良**。ただし
  窓を縮めると鮮度↑とデータ量↓が同時に起きるので **McIntosh & Kamei の反証にはならない**
  ——言えるのは「この実装の sliding は役に立たなかった」まで。

### 規模を上げるまで見えない結論がある(★ v1.83.0、大規模 5 件で再計測)
- prometheus / hugo / etcd / moby / kubernetes(731-13,424 Go ファイル)で再計測。
  **precision@K の飽和は規模の問題だった**——5/5 で scorer が分離する。
  v1.82.0 の「小規模では識別できない」は限界の正しい記述で、指標の欠陥ではない。
- **change coupling は大規模で圧勝**(k=1 で lift 6.0-19.6×、confidence 順が 4/5 で最良)。
  v0.128.0 の「かろうじて勝つ」は小規模の産物だった。ただし **coverage 0.04-0.47**——
  「出せたときは強いが、多くの場合出せない」が正確な要約。
- **relative_churn は 9 リポジトリで一貫して最弱**(kubernetes は lift 0.00)。
  それでも **processrisk の等重みは変えない**: ラベル(次窓で fix commit に触れられた
  ファイル)が大きいファイルを報酬しており、サイズ正規化した唯一の指標が負けるのは
  当然ともとれる。**交絡した測定で製品を曲げない。**

### 定義できない量は null。Inf / NaN を JSON に出さない(★ v1.83.0、tool が沈黙していた)
- `+Inf` / `NaN` は JSON の数値ではない。`encoding/json` は **構造体ごと** marshal に
  失敗するので、1 フィールドが欠けるのではなく **応答が丸ごと消える**。
  `yagura_change_coupling` が hugo k=1 で実際にそうなっていた。
- ゼロ除算で比が定義できないときは `*float64` の nil(= JSON `null`)にし、
  理由を note に書く。**大きな有限値の捏造も、黙って 0 を置くのも同じくらい悪い**
  (0 は「ランダム未満」と読まれる)。
- 教訓は検証側にもある: このバグは `lift <= 1` を落第条件にするテストを
  **+Inf が 1 より大きいので通り抜けていた**。しかもその fixture は意図的に
  baseline を飢えさせていた——**検証していたのは「定義できない値」だった**。
  比を検査するテストは「定義できる fixture」と「定義できない fixture」を必ず対にする。

### 上限に当たったら、上限より先に入力を疑う(★ v1.83.0、誤診で 1 度結論を書きかけた)
- `git log --numstat` が 60 秒上限を超えたとき、製品は「履歴が大きすぎる」と診断し、
  `max_commits` を下げよと言っていた。**下げても直らなかった。**
  真因は `--filter=blob:none` の partial clone で、blob を 1 つずつ取りに行っていた
  (n=50 で 16 秒 / 通常クローンなら 2000 commits が 2.9 秒)。
...
- 文献の答えは **effort-aware 評価**(Arisholm/Briand/Johannessen JSS 2010、
  Mende & Koschke CSMR 2010): 読む LOC の予算 20% で打ち切って recall を測る。
- 同じ 8 リポジトリ・同じ fold で指標だけ替えた結果、**序列が逆転した**:
  `size_loc` は 8/8 でランダム未満、**ManualUp(小さい順)が 8/8 で最良**。
  v1.83.0 の「relative_churn が最弱」は撤回。
- **注記は交絡を打ち消さない。** v1.83.0 は「交絡がある」と書いた上で序列を印刷した。
  交絡があると分かっているなら、序列を出さないか、指標を替えるかのどちらかにする。
- **出荷中の全信号が「小さい順に読む」に 8/8 で負けた。** 重みの配合をいじるのは
  **族ごと負けているものの内部最適化** ——「存在すべきでないものを最適化する」に当たる。
  配合の前に「順位付けを出荷し続けるか」を問う。
- 対照群(ManualUp)を置かない比較は、都合の良い相手としか戦っていない。**常設する。**

### レンズが言語の慣行に反していないか確かめる(★ v1.89.0、3 例目の 100% 偽陽性)
- 未検分だった 8 レンズ・27 件を全部読んだ結果、**`api_doc` の 8 件は 8 件とも偽陽性**。
  doc コメントの付いた `const ( … )` ブロックの中身を「未文書」と報告していた。
- 原因は親 doc の継承を `len(d.Specs) == 1` に限っていたこと。だが **`go doc` は
  グループの doc をメンバーの説明として表示し、golint もブロックに doc があれば咎めない**。
  規則が **Go の慣行そのものに反していた**。修正後 documented_ratio 0.99 → 1.00。
- err_discard(100%)・err_policy(396 件全部テストファイル)に続く **3 例目**。
  3 つとも **件数ではなく finding を読んだ**から見つかった。
- **新しいレンズを書くときは、まず「その言語の慣行では何が正しいのか」を確かめること。**
  型情報を使わない静的規則は、慣行を知らないまま作ると簡単に慣行と衝突する。
- 残り 19 件は真陽性(type_assert 5 / global_check 5 / predeclared 3 / param・return 4 /
  err_wrap・assert_check 2)。**検分は自リポジトリ 1 つ分**であり、
  他リポジトリの偽陽性率は未測定(発見 1 の教訓がそのまま当てはまる)。

### 「決定論的」と書いても、決定論的なのは並び順だけかもしれない(★ v1.88.0)
- `sync_check` は同一入力・同一プロセスで **1 件 ⇄ 26 件** と揺れていた。
  型を **パッケージ修飾なしの名前** で index し、`parsed`(**map**)を走査して
  **代入**していたため、`Config` 等の同名衝突で「どの定義が勝つか」が反復順で決まっていた。
- `Scan` の doc は「出力は決定論的」と明言していた。findings の **並び** は sort して
  いたが、**並べる前の集合**が非決定的だった——**約束は順序、破れは内容**。
- **20 リリース以上見つからなかった理由は規模**。自リポジトリ 352 ファイルには
  パッケージをまたぐ同名 struct がほぼ無く、合成 fixture には皆無。
  precision@K の飽和(v1.83.0)と同じ「規模を上げるまで見えない」型。
- 直し方は **代入 → 和(append)**。反復順に依存せず、方向も正しい
  (見逃し=ロックコピー出荷 の方が過検出より害が大きい)。
  **同名別型の同一視という限界は消さずに明記して残す。**
- 再発防止は **合成 fixture の決定論テストと、実リポジトリ規模の再実行テストを対で置く**
  (`internal/lens/determinism_test.go`、既定 skip)。片方だけでは捕まらない。
- 集合を作ってから並べる処理では、**並びではなく集合の決定性**を検査すること。

### 「効かない対処を指す診断」は型である — 3 度踏んだ(★ v1.87.0)
- 同じ型を、このセッションだけで **3 回** 出荷していた:
  1. `git log` の timeout が「履歴が大きすぎる → max_commits を下げよ」(真因は partial clone、v1.86.0)
  2. 走査の不完全さが `incomplete: true` の真偽値だけ(何%読めたか不明、v1.86.0)
  3. 走査上限が 2 つあるのに、どちらで切れたか言わない(v1.87.0)
- 3 の実測(kubernetes 13,424 ファイル): `max_files` を 5,000 → 25,000 と上げても
  読めるのは **3,843 件のまま**。先に効いているのは **50 MB のバイト上限**。
  **文書化された摘みを上げても何も起きず、なぜかを知る手段が無い。**
- 対策は `truncated_by`(`files`/`bytes`)を全生産点で返すこと。
  **上限自体は上げない**: 1,000 → 5,000 は被覆 7.4% → 28.6% と引き換えに
  discovery call が **10 秒 → 40 秒**。費用はメモリ(22→99 MB)ではなくレイテンシで、
  被覆と速度のどちらが要るかは利用者の状況次第——既定値ひとつでは決められない。
- **型が見えたら個別に潰さず型を潰す。** 新しい診断メッセージを書くときは必ず
  「この対処を実行したら本当に直るか」を自分で一度試すこと。
- 同じ回で Inf/NaN も **seam に機械化**した(`internal/mcp/nonfinite.go`):
  marshal 失敗時に反射で非有限 float の **JSON パスを名指し**し、
  「未定義の比は null」まで言う。人の注意力に頼る再発防止は再発防止ではない。

### gitignore は約束を殺せる — tarball で通ることは repo で通ることではない(★ v1.86.0)
- `.gitignore` の `/yagura-v*/` がソースツリーを丸ごと除外しており、**428 中 191 ファイルが
  git に入っていなかった**: `go.mod`(= clone してビルド不能)、`_test.go` 100 本
  (= 約束を守るはずのテスト)、production `.go` 36 本、`LICENSE`、`.github/workflows/*`。
- **GitHub は `.github/workflows/` を リポジトリのルートでしか読まない。** workflow は
  ソースツリー側の gitignore された場所にあり、Actions の登録 workflow は **0 個**だった
  ——**CI は一度も走っていない**。全 gate は人が手で回していただけ。
- 派生して、長年公開してきた「公開が止まっているのは tag push の 403 のせい」は **誤り**。
  403 は事実だが律速ではない: `release.yml` が登録されていないので、
  **タグを押しても何も起きなかった**。
- 除外の理由は「fake-secret fixture が push protection に引っかかる」。確かめたら **半々**:
  引っかかるのは本当(最初の push が GH013 で拒否、Slack webhook 2 箇所)。
  だが直すのは **2 ファイル 1 行ずつ** で足り、191 件の除外は釣り合っていなかった。
- **思いついた形だけを grep するのは監査ではない。** push 前に 5 パターンを grep して
  「7 ファイル」と結論したが Slack webhook を見ていなかった。自リポジトリに
  **自分の scanner** をかけると 8 ファイル 36 件、push protection が止めたのは 1 件。
  **3 つの検査器が 3 つの違う答えを出す——どの沈黙も証拠にならない。**
- **継承した理由は必ず一度自分で確かめること。ただし確かめ方も疑うこと。**
- 再発防止: `cmd/yagura/repotracked_test.go` が「ビルドに要るものが git に在るか」
  「workflow がルートに在るか」を落ちるテストとして固定する(red を確認してから green)。

### 「データ不足」と診断する前に、測っている量の定義を疑う(★ v1.86.0)
- 未解決項目「費用モデル」は「新しいデータが要る」と書いてあったが、実際に要ったのは
  **新しい費用関数**だった。`cost(f) = file_cost_loc + SizeLOC(f)` を掃引したら、
  **同じ 8 リポジトリ・同じ fold のまま結論が反転した**。
- **ManualUp の 8/8 勝利は `file_cost_loc=0`(ファイルを開くのは無料)の産物**。
  費用 400 では ManualUp が最良なのは 1/8 に落ち、勝者は churn_count / relative_churn /
  complexity / size_loc とリポジトリごとに変わる。lift は 8/8 で単調に低下する。
- **だから既定を反対側に倒すのも誤り。** 400 を支持する実測も持っていない——
  **0 も 400 も等しく根拠がない**。よって費用は **パラメータとして利用者に渡す**
  (`yagura_process_risk` の `file_cost_loc`)。**測っていない量を既定値に隠さない。**
- 一般化: **見出しの主張には必ず「どの仮定の下で」を付ける。** v1.84.0 の
  「出荷中の全信号が負けた」は条件つきの主張だったのに、条件を書いていなかった。

### 数値を疑う人が再実行できないなら、それは主張であって証拠ではない(★ v1.83.0)
- v1.82.0 の再計測は使い捨てハーネスで行い、手順が散文にしか残らなかった。
- v1.83.0 は `internal/walkforward/largeapp_test.go`(`YAGURA_LARGE_DIR` 未設定で skip)
  としてプロトコルを残した。`docs/MULTIREPO_FINDINGS.md` の表はこのテストの出力そのもの。
- **新しい指標や結論を出したら、必ず 2 つ目以降のリポジトリで測ること。** clone は
  この環境からでもできる(実績あり)。実験用の使い捨てコードは残さず、
  再現手順を文書に書く。

### 評価文書は「出典のない形容詞ゼロ」で書く(★ v1.81.0)
- `docs/PRODUCT_ASSESSMENT.md` — 長所・短所・改善点。規律は 1 つ:
  **すべての主張は実測か実装を出典に持つ。** 出典を書けない行は削る。
- 長所は「問い→証拠つきの答え」の形でしか書かない。問いに耐えない項目は
  長所欄に置かず、短所か改善点に落とす(ソクラテス問答の機械化)。
- 改善点は 4 分類で処置する: **実行した / 測って断った / 権限・方針でブロック /
  未解決と明記**。「いつかやる」だけの backlog を作らない。
- backlog を閉じるときは **測って閉じる**: prealloc 31 件は RunAll 実測 4.0 秒
  (支配項は ~18,000 再パース)に対して誤差以下、として数字つきで退役
  (`internal/lens/bench_test.go` が恒久資料)。
- 4 秒を縮める正しい一手はパース共有であってプリアロケートではない——ただし
  純関数レンズ設計の対価を崩すので、**実害が出るまで払わない** と明記する。
...
- 実際の事故: タグ 3 本のうち 2 本(`ｖ1.78.0` / `ｖ1.79.0`)が **全角 'ｖ'(U+FF56)**
  ...
  **release workflow が一度も起動していなかった**。バイナリも SBOM も SLSA provenance も
  生成されないまま「タグは付いている」状態。
- **失敗が「何も起きない」形で現れる** のが最悪の性質。赤いビルドも警告も出ず、
  タグ一覧では ASCII と見分けがつかない。注意力では防げない。
- 対策は `make tag VERSION=x.y.z`(`scripts/tag.sh`)。ASCII `v` + X.Y.Z 以外を拒否し、
  **非 ASCII バイトが 1 つでもあれば拒否**する(regex を後で緩めても全角は通らない)。
- **検証ロジックは shell に 1 本だけ置き、Go のテストは `--check` を実行して確かめる**。
  Go 側に同じ規則を書き写すと、写した方だけが正しくなりうる。
- テスト表には **実際に壊れた 2 つのタグ名** を種として入れること。加えて
  「明らかなゴミを拒否するか」も確認する(常に通る検証器は何も検証しない)。
- `[[ ]]` は keyword なので `LC_ALL=C [[ ... ]]` と前置できない
  (`[[: command not found` になる——最初の実装で踏んだ)。ロケール依存の文字クラスに
  頼らず、非 ASCII バイトの有無で判定する。
- **push はしない**: タグを push する=成果物が世に出る。`make release` と同じく
  外向きの決定は人間に残す。
