...
Status: **v1.88.0** — 79 MCP tools, 96 internal packages, 0 external Go dependencies, **145 consecutive reproducible releases**. Reviewing the previous release's timings showed the same 3,843-file input producing 21,936 / 21,973 / 21,975 findings across runs, against a stated rule of deterministic output. **One lens was answering the same question differently**: `sync_check` indexed types by unqualified name while walking a map, so at kubernetes scale collisions like `Config` let iteration order decide which definition won — 1 finding on one run, 26 on the next. Its doc promised determinism and it sorted the findings; the set they came from was order-dependent. It survived 20+ releases because it **cannot occur at small scale**. Also: `RunAll` is **2.7× faster** (10.1 s → 3.8 s at 1,000 files) by running the pure-function lenses concurrently — no lens signature changed — after the previous release's numbers showed the "it's only 4 seconds" deferral had already expired on a 352-file sample. See [docs/MULTIREPO_FINDINGS.md](docs/MULTIREPO_FINDINGS.md) and [docs/PRODUCT_ASSESSMENT.md](docs/PRODUCT_ASSESSMENT.md). Full lens-by-lens release history: see [CHANGELOG.md](CHANGELOG.md).
