...
Status: **v1.86.0** — 79 MCP tools, 96 internal packages, 0 external Go dependencies, **143 consecutive reproducible releases**. This release began as the last open measurement item and found the repository itself was the defect. **CI had never run once**: all four workflows sat inside a gitignored directory, so GitHub reported **zero registered workflows** — which also means every published claim that releases were blocked by a 403 on tag push was wrong, since there was nothing for a tag to trigger. The same `.gitignore` excluded **191 of 428 files**, `go.mod` among them, so **a clone of this repository could not build**; 100 test files — the ones enforcing every promise made here — were not in it either. All are now tracked and two tests make that falsifiable. The workflows **still cannot be registered** — this session's GitHub App lacks the `workflows` permission — so they are committed, inert, under `ci-workflows-pending/` with the one `git mv` that activates them, and CI remains non-existent until someone with that permission runs it. The measurement item closed too: ManualUp's win over every shipped signal turns out to be **an artifact of assuming files are free to open** — best in 7 of 8 repositories at zero file-open cost, **1 of 8** at 400 LOC — so the cost is now a parameter rather than a hidden default. See [docs/MULTIREPO_FINDINGS.md](docs/MULTIREPO_FINDINGS.md) and [docs/PRODUCT_ASSESSMENT.md](docs/PRODUCT_ASSESSMENT.md). Full lens-by-lens release history: see [CHANGELOG.md](CHANGELOG.md).
...
├── internal/                # 96 packages, none exported
