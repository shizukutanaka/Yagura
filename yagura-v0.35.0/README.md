...
Status: **v0.79.0** — 83 MCP tools, 77 internal packages, 24 computational sensors, shell tab-completion (`yagura completion bash|zsh|fish`). New `predeclared` lens (correctness axis), added after surveying Qiita/Zenn for Go's predeclared-identifier shadowing pitfall (`nishanths/predeclared`): a variable/param/type/func named after a Go builtin (`len`, `cap`, `new`, `error`, `min`/`max`/`clear`, …) silently masks the builtin in that scope. Dogfooded on Yagura: found and fixed **20** real shadowings — all `cap`/`min`/`max` (builtins since Go 1.21) used as local names across 9 files, each renamed to a non-shadowing identifier; now 0. Reproducible build verified.
...
call. This adds nothing to the core — the daemon and the 83 MCP tools are
...
Yagura is agent-agnostic. Its 83 MCP tools work with **any** MCP client, and the
...
74 consecutive releases (v0.6 → v0.79) have shipped with identical SHA-256 across independent builds on the same Go version, `-trimpath`, `-buildvcs=false`, and `CGO_ENABLED=0`.
...
├── internal/                # 77 packages, none exported
