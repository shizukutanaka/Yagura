...
Status: **v1.83.0** — 79 MCP tools, 96 internal packages, 0 external Go dependencies, **140 consecutive reproducible releases**. Closes the assessment's top open item by re-measuring on five large applications (prometheus, hugo, etcd, moby, kubernetes — up to 13,424 Go files): the `precision@K` **saturation reported in v1.82.0 breaks at scale**, all five separate the scorers, and change coupling — called "barely beats a frequency baseline" in v0.128.0 — **wins by 6-20× at k=1**. The measurement also caught two defects in this product: `yagura_change_coupling` returned `+Inf` for an undefined lift, which made `encoding/json` drop **the entire response** (now `null`), and a 1,000-file scan cap reported only `incomplete: true` while reading 7.4% of kubernetes (now `files_read`/`files_total`). See [docs/MULTIREPO_FINDINGS.md](docs/MULTIREPO_FINDINGS.md) and [docs/PRODUCT_ASSESSMENT.md](docs/PRODUCT_ASSESSMENT.md). Full lens-by-lens release history: see [CHANGELOG.md](CHANGELOG.md).
