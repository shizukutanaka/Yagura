...
Status: **v0.36.0** — 62 MCP tools, 52 internal packages, CLI direct mode + `ai-verify`/`quality-check`/`ast-check`, 31 consecutive reproducible releases.
...
call. This adds nothing to the core — the daemon and the 62 MCP tools are
...
Yagura is agent-agnostic. Its 62 MCP tools work with **any** MCP client, and the
...
├── internal/                # 52 packages, none exported
