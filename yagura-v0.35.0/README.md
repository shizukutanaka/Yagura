...
Status: **v1.87.0** — 79 MCP tools, 96 internal packages, 0 external Go dependencies, **144 consecutive reproducible releases**. Closes two open items and finds the same defect a third time. Measuring the 1,000-file scan cap shows **the documented knob is not the one that binds**: above 5,000 files nothing changes because a 50 MB byte cap cuts in first, so raising `max_files` on a large repository does nothing and `incomplete: true` cannot say why — the same shape as v1.86.0's `git log` timeout blaming history size when the cause was a blobless clone. Scans now report `truncated_by`; the cap is deliberately unchanged, because 1,000 → 5,000 buys kubernetes coverage of 7.4% → 28.6% at the price of a discovery call going from **10 s to 40 s**, and that trade is the caller's to make. Separately, Inf/NaN safety moved from human attention to the MCP seam, which now names the JSON path of any non-encodable float and states the fix. See [docs/MULTIREPO_FINDINGS.md](docs/MULTIREPO_FINDINGS.md) and [docs/PRODUCT_ASSESSMENT.md](docs/PRODUCT_ASSESSMENT.md). Full lens-by-lens release history: see [CHANGELOG.md](CHANGELOG.md).
