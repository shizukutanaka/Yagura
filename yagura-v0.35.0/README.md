...
Status: **v1.3.1** — 79 MCP tools, 96 internal packages, 0 external Go dependencies, **134 consecutive reproducible releases**. v1.3.0 found that **503 of ~930** of the product's own lens findings were pure noise (`err_discard` had a 100% false-positive rate; `err_policy`'s 396 were all in test files). This release read the other 27 lenses and found the opposite: **their findings were mostly real** — 360 are threshold readings, the rest true positives — with one fixable class, `flag_arg` reporting single-bool converters like `yesNo(b bool)` as Fowler flag arguments. Seven genuinely ambiguous findings were left reported rather than tuned away. The public surface stays frozen under [docs/COMPATIBILITY.md](docs/COMPATIBILITY.md). Full lens-by-lens release history: see [CHANGELOG.md](CHANGELOG.md).
