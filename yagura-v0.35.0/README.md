...
Status: **v0.75.0** — 79 MCP tools, 73 internal packages, 24 computational sensors, shell tab-completion (`yagura completion bash|zsh|fish`). New `ctxcheck` lens (concurrency axis), added after surveying Qiita/Zenn for Go's `context.Context` conventions: `context-not-first` (ctx must be the first parameter — `containedctx`/golint convention, with the `*testing.T` test-helper exception) and `contained-ctx` (no `context.Context` struct fields — Go blog "Contexts and structs"). Dogfooded on Yagura: caught `writeSSEPinDrift(w, ctx, …)` and fixed it to `(ctx, w, …)`, now 0 violations. Reproducible build verified.
...
70 consecutive releases (v0.6 → v0.75) have shipped with identical SHA-256 across independent builds on the same Go version, `-trimpath`, `-buildvcs=false`, and `CGO_ENABLED=0`.
...
├── internal/                # 73 packages, none exported
