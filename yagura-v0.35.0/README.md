...
Status: **v0.36.0** — 68 MCP tools, 61 internal packages, CLI direct mode + `ai-verify`/`quality-check`/`ast-check`, 31 consecutive reproducible releases.
...
call. This adds nothing to the core — the daemon and the 68 MCP tools are
...
Yagura is agent-agnostic. Its 68 MCP tools work with **any** MCP client, and the
...
├── internal/                # 61 packages, none exported
