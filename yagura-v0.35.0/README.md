...
Status: **v1.3.0** — 79 MCP tools, 96 internal packages, 0 external Go dependencies, **133 consecutive reproducible releases**. This release turned the product's lenses on the product and asked whether their findings were real: **503 of ~930 were pure noise.** `err_discard` had a **100% false-positive rate** — it matched callees by name only, so an internal `Store.Set() error` made it flag every `w.Header().Set(…)`, which returns nothing at all. `err_policy` reported 396 findings, **all** of them in test files where `_ = f.Close()` is normal — and `_ =` is the opposite of a silent drop. Both are now 0, and 16 of 29 lenses report clean. A finding nobody would act on does the same damage as a wrong one. The public surface stays frozen under [docs/COMPATIBILITY.md](docs/COMPATIBILITY.md). Full lens-by-lens release history: see [CHANGELOG.md](CHANGELOG.md).
