...
Status: **v0.36.0** — 64 MCP tools, 57 internal packages, CLI direct mode + `ai-verify`/`quality-check`/`ast-check`, 31 consecutive reproducible releases.
...
call. This adds nothing to the core — the daemon and the 64 MCP tools are
...
Yagura is agent-agnostic. Its 64 MCP tools work with **any** MCP client, and the
...
├── internal/                # 57 packages, none exported
