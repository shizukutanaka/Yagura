...
Status: **v0.36.0** — 63 MCP tools, 56 internal packages, CLI direct mode + `ai-verify`/`quality-check`/`ast-check`, 31 consecutive reproducible releases.
...
call. This adds nothing to the core — the daemon and the 63 MCP tools are
...
Yagura is agent-agnostic. Its 63 MCP tools work with **any** MCP client, and the
...
├── internal/                # 56 packages, none exported
