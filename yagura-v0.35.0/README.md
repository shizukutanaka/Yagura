...
Status: **v0.66.0** — 73 MCP tools, 67 internal packages, 24 computational sensors, shell tab-completion (`yagura completion bash|zsh|fish`). New `flag-arg` lens (Fowler "Flag Argument") — bool params encoding hidden control-flow branches; closes the blind spot where complexity + paramcheck miss semantic coupling. Dogfooded: fixed 5 true Fowler smells, reduced findings 18→13. Reproducible build verified (byte-for-byte identical via `make verify`).
...
call. This adds nothing to the core — the daemon and the 73 MCP tools are
...
Yagura is agent-agnostic. Its 73 MCP tools work with **any** MCP client, and the
...
61 consecutive releases (v0.6 → v0.66) have shipped with identical SHA-256 across independent builds on the same Go version, `-trimpath`, `-buildvcs=false`, and `CGO_ENABLED=0`.
...
├── internal/                # 67 packages, none exported
