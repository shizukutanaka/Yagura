...
Status: **v1.84.0** — 79 MCP tools, 96 internal packages, 0 external Go dependencies, **141 consecutive reproducible releases**. Adds **effort-aware evaluation** (Arisholm et al. JSS 2010; Mende & Koschke CSMR 2010) to remove a size confound in the defect-prediction labels — and the scorer ordering **inverts**: ranking by file size, best on `precision@K` in 4 of 5 repositories, is **below random in 8 of 8** once you pay for the lines you read, while `relative_churn`, previously "weakest in 5 of 5", beats random in 4 of 8. **v1.83.0's ordering is retracted.** The result we did not want: **reading the smallest files first beat every signal this project ships, on all 8 repositories** — so the walk-forward note now says so, before you trust a ranking from it. See [docs/MULTIREPO_FINDINGS.md](docs/MULTIREPO_FINDINGS.md) and [docs/PRODUCT_ASSESSMENT.md](docs/PRODUCT_ASSESSMENT.md). Full lens-by-lens release history: see [CHANGELOG.md](CHANGELOG.md).
