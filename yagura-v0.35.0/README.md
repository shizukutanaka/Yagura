...
Status: **v0.36.0** — 67 MCP tools, 60 internal packages, CLI direct mode + `ai-verify`/`quality-check`/`ast-check`, 31 consecutive reproducible releases.
...
call. This adds nothing to the core — the daemon and the 67 MCP tools are
...
Yagura is agent-agnostic. Its 67 MCP tools work with **any** MCP client, and the
...
├── internal/                # 60 packages, none exported
