...
Status: **v1.85.0** — 79 MCP tools, 96 internal packages, 0 external Go dependencies, **142 consecutive reproducible releases**. v1.84.0 found that every signal shipped here loses to the trivial "read the smallest files first" baseline under effort-aware evaluation; this release finds out **why, and fixes it**. The defect was never the weighting — it was the normalization. Big files have more of everything, so summing raw counts mistakes *large* for *risky*: raw `churn_count` beats random ordering in **0 of 8** repositories, and `churn_count` **per LOC** beats it in **8 of 8**. `processrisk` now divides count signals by file size before ranking. **It still does not beat the trivial baseline** (mean effort lift 1.61 vs 1.68) — the tool's own note says so, because a ranking earns its place by naming *which* files, not by winning on recall. See [docs/MULTIREPO_FINDINGS.md](docs/MULTIREPO_FINDINGS.md) and [docs/PRODUCT_ASSESSMENT.md](docs/PRODUCT_ASSESSMENT.md). Full lens-by-lens release history: see [CHANGELOG.md](CHANGELOG.md).
