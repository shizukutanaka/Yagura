...
Status: **v1.3.2** — 79 MCP tools, 96 internal packages, 0 external Go dependencies, **135 consecutive reproducible releases**. This release investigated whether `hotspot` — the convergence lens — had gone stale again (it once sat at 19% coverage unnoticed, and expanding it took convergent hotspots from 0 to 69). **The hypothesis was wrong:** only 13 of 29 lenses are function-keyed, and hotspot bundles 12 of the 12 applicable ones. What was missing was any guard against recurrence — the population was a hardcoded list, and a new lens sitting outside it would read as “the code got cleaner” rather than “the measurement got weaker”. That guard now exists. The public surface stays frozen under [docs/COMPATIBILITY.md](docs/COMPATIBILITY.md). Full lens-by-lens release history: see [CHANGELOG.md](CHANGELOG.md).
